	...
	$.PFold.prototype = {
		...
		_start : function( action, step ) {
				...
				case 0 : case this.options.folds - 1 :
					...
					if( action === 'fold' ) {
						...
						if( step === this.options.folds - 1 ) {
							//console.log(['step === this.options.folds - 1', step])
						...
						if( step === 0 ) {
							...
							if (this.options.centered) {
								...
								this.$el.addClass( 'uc-current' ).css( { left : 0, top : 0 } );
							}
			...
			if( this.support ) {
				...
				if (action === 'fold') { styleCSS.left = -5555; /*console.log(styleCSS);*/ }
			}
		...
		_setLastStep : function( direction, styleCSS ) {
			...
			var contentTopBackClasses = direction === 'top' || direction === 'bottom' ? 'uc-inner uc-inner-rotate' : 'uc-inner';
				...
				//console.log([contentBottom, contentTopBack])
			return {
